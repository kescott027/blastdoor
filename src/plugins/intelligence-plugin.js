...
function normalizeManagerString(value, fallback = "") {
...
const SAFE_WIZARD_ACTIONS = new Set(["snapshot.network", "check.gateway-local", "detect.wsl-portproxy"]);
...
function createWizardHostFingerprint({ environment = {}, config = {} } = {}) {
...
function normalizeWizardQuestionList(value) {
  if (!Array.isArray(value)) {
    return [];
  }
  return value
...
function normalizeWizardAnswerList(value) {
  if (!Array.isArray(value)) {
    return [];
  }
  return value
...
function normalizeWizardState(input = {}, defaults = {}) {
...
function createDefaultWizardState({ workflowId, hostFingerprint, runName = "" } = {}) {
...
function buildWizardSummary(run) {
  const sourceRun = run && typeof run === "object" ? run : {};
...
function parseClarificationContract(result = {}) {
...
function parseSufficiencyContract(result = {}) {
  const source = result && typeof result === "object" ? result : {};
  return {
...
function parseExecutionPlanContract(result = {}) {
  const source = result && typeof result === "object" ? result : {};
...
function normalizeSafeActionTrustList(value) {
  if (!Array.isArray(value)) {
    return [];
  }
  const out = [];
...
function workflowTrustsAction({ workflow, actionId, hostFingerprint }) {
